...
opaklik = global.envanter_ekrani_gorunme && global.envanter_sekme == global.sekme_numara_stats;
image_alpha = opaklik;
...
if point_distance(x, y, mouse_x, mouse_y) < mouse_yakinlik && opaklik && !duraklatmak {
	...
	if mouse_check_button_pressed(mb_left) {
		if sprite_index = spr_ok_yukari {
			if global.stat_ekrani_sayfa > 1 {
				global.stat_ekrani_sayfa -= 1;	
			}
		}
		if sprite_index = spr_ok_asagi {
			if global.stat_ekrani_sayfa < global.stat_son_sayfa {
				global.stat_ekrani_sayfa += 1;
			}
		}
	}
